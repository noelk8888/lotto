...
test('uses six numbers after a label despite tilt and adjacent receipt numbers', () => {
  const values = ['05','09','11','21','30','36'];
  const boxes = [word('A', 0, 10), word(':', 14, 10),
...
test('LP has no effect on a labelled row', () => {
  const boxes = [word('B', 0, 20), ...['08','16','18','19','24','32'].map((value, index) => word(value, 25 + index * 20, 20))];
  assert.equal(labelledRows(boxes), 'B: 08 16 18 19 24 32');
  assert.equal(labelledRows([...boxes, word('LP', 160, 20)]), 'B: 08 16 18 19 24 32');
});
